 ...
 * sendmsg() interface.
 */
INT32 sctp_sendmsg(INT32 s, const VOID *msg, size_t len, sock_addr_t *to, socklen_t tolen,
		UINT32 ppid, UINT32 flags, UINT16 stream_no, UINT32 timetolive, UINT32 context) {
 ...
 */
INT32 sctp_send(INT32 s, const VOID *msg, size_t len, const struct sctp_sndrcvinfo *sinfo,
		INT32 flags) {
	struct msghdr outmsg;
	struct iovec iov;
