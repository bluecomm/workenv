 ...
 */
enum sctp_sac_state {
	SCTP_COMM_UP, SCTP_COMM_LOST, SCTP_RESTART, SCTP_SHUTDOWN_COMP, SCTP_CANT_STR_ASSOC,
};
...
enum sctp_spp_flags {
	...
	SPP_HB = SPP_HB_ENABLE | SPP_HB_DISABLE, SPP_HB_DEMAND = 1 << 2, /*Send heartbeat immediately*/
	SPP_PMTUD_ENABLE = 1 << 3, /*Enable PMTU discovery*/
	SPP_PMTUD_DISABLE = 1 << 4, /*Disable PMTU discovery*/
	SPP_PMTUD = SPP_PMTUD_ENABLE | SPP_PMTUD_DISABLE, SPP_SACKDELAY_ENABLE = 1 << 5, /*Enable SACK*/
	SPP_SACKDELAY_DISABLE = 1 << 6, /*Disable SACK*/
	SPP_SACKDELAY = SPP_SACKDELAY_ENABLE | SPP_SACKDELAY_DISABLE, SPP_HB_TIME_IS_ZERO = 1
			<< 7, /* Set HB delay to 0 */
};
...
enum {
	SCTP_AUTH_HMAC_ID_SHA1 = 1, SCTP_AUTH_HMAC_ID_SHA256 = 3,
};
...
enum sctp_spinfo_state {
	SCTP_INACTIVE, SCTP_ACTIVE, SCTP_UNCONFIRMED
};
...
int32_t sctp_bindx(int32_t sd, sock_addr_t *addrs, int32_t addrcnt, int32_t flags);

int32_t sctp_connectx(int32_t sd, sock_addr_t *addrs, int32_t addrcnt, sctp_assoc_t *id);

int32_t sctp_peeloff(int32_t sd, sctp_assoc_t assoc_id);

/* Prototype for the library function sctp_opt_info defined in
 * API 7. Socket Options.
 */
int32_t sctp_opt_info(int32_t sd, sctp_assoc_t id, int32_t opt, void *arg, socklen_t *size);
 ...
 */
int32_t sctp_getpaddrs(int32_t sd, sctp_assoc_t id, sock_addr_t **addrs);
 ...
 */
int32_t sctp_freepaddrs(sock_addr_t *addrs);
 ...
 * This is implemented using the getsockopt() interface.
 */
int32_t sctp_getladdrs(int32_t sd, sctp_assoc_t id, sock_addr_t **addrs);
 ...
 */
int32_t sctp_freeladdrs(sock_addr_t *addrs);
 ...
 * sendmsg() interface.
 */
int32_t sctp_sendmsg(int32_t s, const void *msg, size_t len, sock_addr_t *to, socklen_t tolen,
 ...
 */
int32_t sctp_send(int32_t s, const void *msg, size_t len, const struct sctp_sndrcvinfo *sinfo,
		int32_t flags);
 ...
 * recvmsg() interface.
 */
int32_t sctp_recvmsg(int32_t s, void *msg, size_t len, sock_addr_t *from, socklen_t *fromlen,
		struct sctp_sndrcvinfo *sinfo, int32_t *msg_flags);

/* Return the address length for an address family. */
int32_t sctp_getaddrlen(sa_family_t family);
