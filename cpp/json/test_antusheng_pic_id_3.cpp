...
bool parse_complex_json(const std::string& json_str, std::vector<std::string>& pic_urls) {
    ...
    try{  
        boost::property_tree::ptree root_item_pt = root_pt.get_child("item");
        std::cout << "item array size: " << root_item_pt.size() << std::endl;
        // for each item in "item" array
        for (boost::property_tree::ptree::iterator it_item = root_item_pt.begin();
                it_item != root_item_pt.end(); ++it_item) {
...
int main() {
    const std::string input_json_file = "antusheng_3.txt";
