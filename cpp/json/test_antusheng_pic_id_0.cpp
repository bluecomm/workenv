...
bool parse_complex_json(const std::string& json_str, std::vector<std::string>& pic_urls) {
    ...
    try {
        // get "reserved -> image_source"
        boost::property_tree::ptree reserved_pt = root_pt.get_child("reserved");
        std::string image_source_str = reserved_pt.get<std::string>("image_source");
    ...
    try{
        // get "item" array
        boost::property_tree::ptree item_array_pt = root_pt.get_child("item");
        BOOST_FOREACH(boost::property_tree::ptree::value_type &v, item_array_pt) {
...
int main() {
    const std::string input_json_file = "antusheng_0.txt";
